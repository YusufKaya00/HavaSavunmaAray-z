			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						// Bolt optimization: Use BeginInvoke with a flag to prevent flooding the UI thread and avoid blocking the background thread.
						// Update both picture boxes in a single asynchronous UI call.
						...
						{
							// Avoid memory leak if we skip UI update for these frames
							rawBmp.Dispose();
