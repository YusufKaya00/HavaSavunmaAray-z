...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		private volatile bool _isUpdatingUI = false; // Bolt: Flow control flag for UI updates
			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						{
							frameCount++;
							// Bolt optimization: Flow control to prevent message queue flooding and OutOfMemoryException
							// Combine updates to minimize context switches, use BeginInvoke so background loop doesn't block
							if (!_isUpdatingUI)
							{
								_isUpdatingUI = true;
							...
							{
								// Drop frame, dispose bitmaps to avoid memory leak when UI is too busy
								rawBmp.Dispose();
