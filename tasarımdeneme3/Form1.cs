...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		// Bolt optimization: Flow control flag to drop UI frames and prevent message queue flooding
		private volatile bool isUpdatingUI = false;
			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						// Bolt optimization: Frame dropping with BeginInvoke.
						// Instead of synchronously waiting with Invoke and updating UI twice per loop,
						// we combine updates and skip frame rendering if the UI thread is still processing the previous request.
						// This prevents flooding the WinForms message queue which leads to memory bloat.
						if (this.IsHandleCreated && !isUpdatingUI)
						{
							isUpdatingUI = true;
							...
							BeginInvoke(new Action(() =>
							{
								try
								{
									var oldRaw = pictureBox2.Image;
									var oldProcessed = pictureBox3.Image;

									pictureBox2.Image = rawBmp;
									pictureBox3.Image = processedBmp;
						...
						{
							// If we skipped the UI update, we must dispose the bitmaps immediately to prevent memory leak
							rawBmp.Dispose();
