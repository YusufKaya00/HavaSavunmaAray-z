...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		private volatile bool isUpdatingUI = false; // Flow control for UI updates
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						if (this.IsHandleCreated && !isUpdatingUI)
						{
							...
							// Bolt optimization: Use BeginInvoke with a flow-control flag to prevent message queue flooding,
							// avoiding OutOfMemoryException and combining updates for efficiency.
							BeginInvoke(new Action(() =>
							{
								...
								isUpdatingUI = false; // Allow next frame update
							}));
						}
