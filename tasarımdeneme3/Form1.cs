...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		private const int FrameHeight = 640;
			private volatile bool isUpdatingUI_raw = false;
		private volatile bool isUpdatingUI_processed = false;
	...
	private void ClockTimer_Tick(object sender, EventArgs e)
			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						if (this.IsHandleCreated && !isUpdatingUI_raw)
						{
							frameCount++;
							isUpdatingUI_raw = true;
						...
						if (this.IsHandleCreated && !isUpdatingUI_processed)
						{
							isUpdatingUI_processed = true;
							BeginInvoke(new Action(() =>
							{
								try
								{
		...
		private void StartPythonScript()
		{
			try
			{
				...
				pythonProcess.StartInfo.Arguments = @"anascriptArayüz.py"; // Betiğin tam yolu
				pythonProcess.StartInfo.WorkingDirectory = @"."; // Çalışma dizini
