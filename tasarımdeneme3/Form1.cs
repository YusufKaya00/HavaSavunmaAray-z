		...
		private void ReadSharedMemory()
		{
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					// Bolt optimization: Add a flag to drop UI frames if the main thread is busy and prevent message queue flooding.
					...
					while (true)
					{
						...
						if (this.IsHandleCreated && !isUpdatingUI)
						{
							isUpdatingUI = true;
							frameCount++;
							// Bolt optimization: Use BeginInvoke to prevent blocking the high-frequency loop.
						...
						{
							// Dispose unused bitmaps if UI frame is dropped
							rawBmp.Dispose();
