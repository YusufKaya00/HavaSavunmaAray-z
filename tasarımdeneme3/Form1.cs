...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		// Bolt optimization: Flow control flag for UI updates from background threads
			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						// Bolt optimization: Flow control to drop frames if UI is busy, and batching UI updates
						// to avoid flooding the message queue and causing OutOfMemoryException
						...
						{
							// If dropping frame, dispose bitmaps to avoid memory leaks
							rawBmp.Dispose();
