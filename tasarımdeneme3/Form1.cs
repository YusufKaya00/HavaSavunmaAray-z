...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		private int _isUpdatingUIRaw = 0; // Bolt optimization: Flow control for raw frame UI updates (Interlocked)
		private int _isUpdatingUIProcessed = 0; // Bolt optimization: Flow control for processed frame UI updates (Interlocked)
	...
	private void ClockTimer_Tick(object sender, EventArgs e)
		...
		private void ReadSharedMemory()
		{
			try
			{
				...
				// Bolt optimization: Pre-allocate MemoryMappedViewAccessor and large byte arrays outside the high-frequency loop
				// to avoid severe Garbage Collection (GC) pressure and Large Object Heap (LOH) fragmentation.
				...
				{
					// Bolt optimization: Pre-allocate large buffers outside loop
					...
					while (true)
					{
						...
						// Bolt optimization: Asynchronous non-blocking UI update with flow control to prevent message queue flooding.
						if (this.IsHandleCreated && System.Threading.Interlocked.CompareExchange(ref _isUpdatingUIRaw, 1, 0) == 0)
						{
							frameCount++;
							BeginInvoke(new Action(() =>
							{
								...
								{
									System.Threading.Interlocked.Exchange(ref _isUpdatingUIRaw, 0);
								}
							}));
						}
						else
						{
							// If dropping frame, dispose it immediately to avoid memory leaks
							rawBmp.Dispose();
						}
						...
						// Bolt optimization: Asynchronous non-blocking UI update with flow control to prevent message queue flooding.
						if (this.IsHandleCreated && System.Threading.Interlocked.CompareExchange(ref _isUpdatingUIProcessed, 1, 0) == 0)
						{
							BeginInvoke(new Action(() =>
							{
								try
								...
								{
									System.Threading.Interlocked.Exchange(ref _isUpdatingUIProcessed, 0);
								}
							}));
						}
						else
