	...
	private void ClockTimer_Tick(object sender, EventArgs e)
		...
		private void ReadSharedMemory()
		{
			try
			{
				...
				{
				using (var rawAccessor = rawMmf.CreateViewAccessor()) // ⚡ Bolt: ViewAccessor is created outside the loop
				using (var processedAccessor = processedMmf.CreateViewAccessor()) // ⚡ Bolt: ViewAccessor is created outside the loop
				{
					// ⚡ Bolt: Memory allocation is moved outside the loop.
					// This avoids creating 2 arrays of size 640*640*3 (1.2MB each) at 60 FPS, reducing GC pressure by ~144 MB/s.
