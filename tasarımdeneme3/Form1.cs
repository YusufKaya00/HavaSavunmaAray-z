...
namespace tasarımdeneme3
{
	...
	private void ClockTimer_Tick(object sender, EventArgs e)
			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						if (this.IsHandleCreated && !isUpdatingUIRaw)
						{
							frameCount++;
							isUpdatingUIRaw = true;
							BeginInvoke(new Action(() =>
							{
								pictureBox2.Image?.Dispose();
								pictureBox2.Image = rawBmp; // İşlenmemiş görüntü
								isUpdatingUIRaw = false;
							}));
						}
						else
						{
							// Drop frame to prevent flooding UI thread
							rawBmp.Dispose();
						}
						...
						if (this.IsHandleCreated && !isUpdatingUIProcessed)
						{
							isUpdatingUIProcessed = true;
							BeginInvoke(new Action(() =>
							{
								pictureBox3.Image?.Dispose();
								pictureBox3.Image = processedBmp; // İşlenmiş görüntü
								isUpdatingUIProcessed = false;
							}));
						}
						else
						{
		...
		// Bolt optimization: Flow control flags for UI updates to drop frames if WinForms message queue is busy
		private bool isUpdatingUIRaw = false;
		private bool isUpdatingUIProcessed = false;
