...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		private volatile bool _isUpdatingUI = false; // Bolt optimization: Flow control for UI updates
			...
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (_isRunning)
					{
						...
						if (this.IsHandleCreated && !_isUpdatingUI)
						{
							_isUpdatingUI = true;
							frameCount++;

							// Bolt optimization: Only process Bitmaps if we are actually going to update the UI.
							...
							// Bolt optimization: Use asynchronous BeginInvoke and unify updates to avoid flooding message queue
