	...
	private void ClockTimer_Tick(object sender, EventArgs e)
		...
		private void ReadSharedMemory()
		{
			try
			{
				...
				{
					// ⚡ Bolt Optimization: Reuse MemoryMappedViewAccessor and byte arrays.
					// 💡 What: Moved rawAccessor, processedAccessor, rawBuffer and processedBuffer declarations out of the loop.
					// 🎯 Why: Previously, creating accessors and allocating 1.2MB byte arrays x 2 per loop iteration (every 15ms)
					// resulted in high Garbage Collector pressure, allocating ~2.4MB memory continuously.
					// 📊 Impact: Significantly reduces GC collections and CPU overhead per frame processing.
					using (var rawAccessor = rawMmf.CreateViewAccessor())
					using (var processedAccessor = processedMmf.CreateViewAccessor())
					{
						// Pre-allocate buffers once to prevent continuous 1.2MB allocations every 15ms
