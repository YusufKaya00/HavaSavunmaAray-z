		...
		private void ReadSharedMemory()
		{
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					// Bolt optimization: Flow control flag to avoid flooding the UI thread message queue
					bool isUpdatingUI = false;

					while (true)
					{
						// Bolt optimization: Drop frames if the UI thread is still busy processing the previous frame.
						// This prevents OutOfMemoryExceptions and WinForms message queue bloat.
						if (isUpdatingUI)
						{
							Thread.Sleep(5); // Yield briefly
							continue;
						}
						...
						if (this.IsHandleCreated)
						{
							frameCount++;
							isUpdatingUI = true;
							// Bolt optimization: Use BeginInvoke to update asynchronously and combine updates to reduce overhead.
