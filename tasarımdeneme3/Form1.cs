...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		// Bolt optimization: Flow control flag for UI updates from shared memory thread
		...
		private void ReadSharedMemory()
		{
			try
			{
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						// Bolt optimization: Drop frame if UI is still processing the previous one
						// This prevents flooding the WinForms message queue which causes memory bloat and OOM
						if (isUpdatingUI)
						{
							Thread.Sleep(5);
							continue;
						...
						{
							isUpdatingUI = true;
							// Bolt optimization: Use asynchronous BeginInvoke and batch UI updates to avoid blocking the high-frequency background thread
