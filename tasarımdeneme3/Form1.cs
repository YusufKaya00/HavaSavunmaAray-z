...
namespace tasarımdeneme3
{
	public partial class Form1 : Form
	{
		...
		// Bolt optimization: Track if UI is currently updating to avoid flooding the message queue
				...
				//using (var arduinoDataMmf = MemoryMappedFile.OpenExisting("ArduinoData")) // ArduinoData için shared memory
				{
					...
					while (true)
					{
						...
						if (this.IsHandleCreated && !isUpdatingUI)
						{
							isUpdatingUI = true;
							frameCount++;
							// Bolt optimization: Combine UI updates and use asynchronous BeginInvoke to prevent blocking
							// the high-frequency shared memory loop, avoiding duplicate context switching overhead.
							BeginInvoke(new Action(() =>
							{
								...
								isUpdatingUI = false; // Reset the flag once UI is updated
							}));
						}
						else
						{
							// If UI is still updating, drop this frame to prevent message queue flooding and memory leaks
							rawBmp?.Dispose();
